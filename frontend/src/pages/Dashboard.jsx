...
import { Activity, AlertTriangle, FolderKanban, ArrowUpRight, ArrowDownRight, ScanLine, Clock, ShieldAlert, Moon, Sun } from 'lucide-react';
    ...
    <PageTransition>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold text-main mb-1">Dashboard</h1>
            <p className="text-muted">Welcome to your accessibility overview.</p>
          </div>
          <div className="flex items-center gap-3">
            <button 
              onClick={toggleTheme}
              className="p-2.5 bg-surface border border-border-subtle text-muted hover:text-main rounded-xl transition-all shadow-sm"
            >
              {theme === 'light' ? <Moon size={20} /> : <Sun size={20} />}
            </button>
            <button className="flex items-center gap-2 bg-primary text-base px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-primary/40 hover:bg-primary/90 hover:-translate-y-0.5 transition-all">
              <ScanLine size={18} />
              New Scan
            </button>
          </div>
        ...
        >
          ...
          <motion.div variants={itemVariants} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 p-6 rounded-2xl hover:bg-slate-800/80 transition-colors group">
            ...
            <h3 className="text-muted font-medium mb-1">Avg Accessibility Score</h3>
            <h2 className="text-3xl font-bold text-main">88<span className="text-xl text-muted">/100</span></h2>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 p-6 rounded-2xl hover:bg-slate-800/80 transition-colors group">
            ...
            <h3 className="text-muted font-medium mb-1">Unresolved Issues</h3>
            <h2 className="text-3xl font-bold text-main">137</h2>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 p-6 rounded-2xl hover:bg-slate-800/80 transition-colors group">
            ...
            <h3 className="text-muted font-medium mb-1">Active Projects</h3>
            <h2 className="text-3xl font-bold text-main">12</h2>
          </motion.div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
          ...
          <motion.div 
            ...
            className="lg:col-span-2 bg-surface backdrop-blur-sm border border-border-subtle p-6 rounded-2xl"
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-main">Score Trend</h3>
              <select className="bg-surface-deep border border-border-subtle text-muted text-sm rounded-lg focus:ring-primary focus:border-primary px-3 py-1.5 outline-none">
          ...
          <motion.div 
            ...
            className="lg:col-span-1 bg-surface backdrop-blur-sm border border-border-subtle p-6 rounded-2xl flex flex-col"
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-main">Recent Scans</h3>
