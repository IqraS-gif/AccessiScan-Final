...
def extract_json(content: str):
    ...
    try:
        ...
        if "```json" in content:
            ...
            # Check if there's any text before the first backticks
            parts = content.split("```")
            if len(parts) >= 3:
                content = parts[1].strip()
                # If it starts with 'json', strip it
    ...
    try:
        ...
        if match:
            ...
            # Basic repair: remove common trailing commas before closing brackets
            json_str = re.sub(r',\s*([\]\}])', r'\1', json_str)
            return json.loads(json_str)
    except:
        pass
    
    # 4. Last resort: split by brackets manually if regex failed
    try:
    ...
    raise ValueError(f"Could not extract valid JSON from AI response. Content: {content[:200]}...")
...
Return ONLY a RAW JSON list of 5 issues. 
NO CONVERSATION. NO EXPLANATIONS. NO MARKDOWN BLOCKS if possible, but if you use them, ensure the JSON inside is perfect.
...
def build_minimal_rules(rules: list) -> list:
    ...
    for rule in rules:
        ...
        minimal.append({
            ...
            # "principle": rule.get("principle"), # Removed to save tokens
            "description": rule.get("description", "")[:100], # Reduced from 200
            "act_checks": act_titles[:2] # Limited to top 2
        })
    return minimal
...
{{
  ...
  "final_summary": {{
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "You are an expert accessibility auditor. Output only valid JSON. Be extremely concise and follow the requested schema exactly."},
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        for loc in locations:
            ...
            try:
                ...
                print(f"[WARN] GitHub analysis failed for location '{loc}': {e}")
                continue
